...
The raw data was imported into R [@R-base] where it was tidied, filtered using the *tidyverse* package [@R-tidyverse], and combined using the *dplyr* package [@R-dplyr]. Data from the 'visit' data set was filtered out for `mri_numer` as it held the same information as the `visit` column, which was preferred to be the kept column as it is numerical so posed as a better variable for future analysis through pca and other visualisations. 
...
The first graph used the active data to examine how much variance each component accounts for.
